...
for frame in frames:
	...
	for i in range(len(tasks)):
		...
		if scale == "lin":
			...
			field_bins = numpy.linspace(0.999*minval, 1.001*maxval, N)
		elif scale == "log":
			N = 1000
			field_bins = numpy.logspace(numpy.log10(0.999*minval), numpy.log10(1.001*maxval), N)
		elif scale == "symlog":
			N = 1000
			absmax = max(abs(minval), abs(maxval))
			N2 = max(100, int(N*dx/absmax))
			field_bins = numpy.logspace(numpy.log10(1.001*dx), numpy.log10(1.001*absmax), N)
			field_bins = numpy.concatenate(((-field_bins)[::-1], numpy.linspace(-dx, dx, N2), field_bins))
